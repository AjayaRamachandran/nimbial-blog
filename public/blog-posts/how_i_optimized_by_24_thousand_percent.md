...
---
In October 2025, I released Symphony v1.0, the music editor designed for beginners, to the world.

Symphony is a cross-platform desktop app, which was, by public release 1.0.2, stable with minimal unexplained bugs or crashes. But one huge problem still plagued the program: ***the boot time.***

**It took over *2 minutes to boot*.**
...
> **Side note here:**
    At this point many people suggest searching Google, asking LLMs, or StackOverflow. And while these places often give great advice, it's often general advice that solves *general* problems. As an example, Google or ChatGPT might tell you that slow startup can be fixed with aggressive caching, multi-process "lazy loading", or other slap-on-top solutions. But my vehement critique of these methods runs deep; not only do they prevent critical thinking and the ideation of *new* solutions, they're almost designed to get you deeper in technical debt (in this case, see how the solutions frame slowness as a side-effect to be suppressed, rather than an architectural problem). So, my personal and professional opinion: don't start with these triage methods. Learn your codebase inside and out — every line of code. I promise you it's worth the time and energy, and you'll see how it genuinely saved my ass later in this post.
---
...
Next, I performed a "controlled experiment": I compared the boot times (using a stopwatch — *very high-tech*) of the unpackaged Python script and the packaged app on my computer and on the computer it was very slow on.
...
##### This has its benefits, like being able to dynamically import regular Python `.py` files as modules *post-build*, or providing a basic assurance that building the app doesn't induce any behavioral differences in the app. But by no means is it efficient or elegant.
...
At this point, it didn't matter to me *why*; the reason could have been any number of things, and it was most likely not something I could improve without massively expanding the scope of a hobby project. But it was clear where we had to go from here:

- Step 1: Boot the Python program ONCE instead of launching a new instance for each .symphony file
- Step 2: Create a live mode of communication between the PM and editor that is more stable and customizable than sysargs
- Step 3: Treat opening, reading, and instantiating .symphony files as simple instructions to an already-running program, not a new instance.
